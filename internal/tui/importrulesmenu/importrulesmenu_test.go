...
func TestNewImportRulesModel(t *testing.T) {
	...
	// them all — there is nothing to import, so the model must come up in an
	...
	t.Run("no repositories configured", func(t *testing.T) {
		...
		empty := NewImportRulesModel(helpers.NewUIContext(80, 24, cfg, logger))
...
func TestNoUsableRepositoriesError(t *testing.T) {
	...
	if single := noUsableRepositoriesError(1).Error(); !strings.Contains(single, "1 configured repository") {
